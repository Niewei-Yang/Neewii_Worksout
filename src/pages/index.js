...
import { filterAndSortRuns, filterCityRuns, filterTitleRuns, filterTypeRuns, filterYearRuns, geoJsonForRuns, getBoundsForGeoData, scrollToMap, sortDateFunc, titleForShow, } from '@/utils/utils';
...
const Index = () => {
    ...
    const [animationRuns, setAnimationRuns] = useState([]);
    const [currentFilter, setCurrentFilter] = useState({ item: thisYear, func: filterYearRuns, item2: null, func2: null });
    // State to track if we're showing a single run from URL hash
    const [singleRunId, setSingleRunId] = useState(null);
    // Animation trigger for single runs - increment this to force animation replay
    const [animationTrigger, setAnimationTrigger] = useState(0);
    const selectedRunIdRef = useRef(null);
    const selectedRunDateRef = useRef(null);
    const previousRunsRef = useRef(null);
    ...
    const runs = useMemo(() => {
        return filterAndSortRuns(activities, currentFilter.item, currentFilter.func, sortDateFunc, currentFilter.item2, currentFilter.func2);
    }, [
        activities,
        currentFilter.item,
        currentFilter.func,
    ...
    const startAnimation = useCallback((runsToAnimate) => {
    ...
    const changeByItem = useCallback((item, name, func) => {
        scrollToMap();
        if (name != 'Year') {
            setYear(thisYear);
        }
    ...
    const changeTypeInYear = useCallback((year, type) => {
        scrollToMap();
    ...
    const setActivity = useCallback((_newRuns) => {
    ...
    const locateActivity = useCallback((runIds) => {
        const ids = new Set(runIds);
        const selectedRuns = !runIds.length
            ? runs
            : runs.filter((r) => ids.has(r.run_id));
    ...
    return (_jsxs(Layout, { children: [_jsx(Helmet, { children: _jsx("html", { lang: "en", "data-theme": theme }) }), _jsxs("div", { className: "w-full lg:w-1/3", children: [_jsx("h1", { className: "my-12 mt-6 text-5xl font-extrabold italic", children: _jsxs("a", { href: siteUrl, "aria-label": siteTitle, className: "inline-flex flex-col leading-none", children: [_jsx("span", { className: "whitespace-nowrap", children: "Neewii\u7684" }), _jsx("span", { className: "whitespace-nowrap", children: "Workouts Map" })] }) }), (viewState.zoom ?? 0) <= 3 && IS_CHINESE ? (_jsx(LocationStat, { changeYear: changeYear, changeCity: changeCity, changeType: changeType, onClickTypeInYear: changeTypeInYear })) : (_jsx(YearsStat, { year: year, onClick: changeYear, onClickTypeInYear: changeTypeInYear }))] }), _jsxs("div", { className: "w-full lg:w-2/3", id: "map-container", children: [_jsx(RunMap, { title: title, viewState: viewState, geoData: animatedGeoData, setViewState: setViewState, changeYear: changeYear, thisYear: year, animationTrigger: animationTrigger }), year === 'Total' ? (_jsx(SVGStat, {})) : (_jsx(RunTable, { runs: runs, locateActivity: locateActivity, setActivity: setActivity, runIndex: runIndex, setRunIndex: setRunIndex }))] }), import.meta.env.VERCEL && _jsx(Analytics, {})] }));
};
