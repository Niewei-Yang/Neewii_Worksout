...
const CHINESE_INFO_MESSAGE = (yearLength, year) => `距首次记录过去了 ${yearLength} 年 ` +
    (year === 'Total' ? '' : `，地图展示的是 ${year} 年的轨迹`);
const ENGLISH_INFO_MESSAGE = (yearLength, year) => `Logged ${yearLength} Years of Outdoor Journey` +
    (year === 'Total' ? '' : `, the map show routes in ${year}`);
// English is not supported for location info messages yet
const CHINESE_LOCATION_INFO_MESSAGE_FIRST = '我去过了一些地方，希望随着时间推移，地图点亮的地方越来越多';
...
export { USE_GOOGLE_ANALYTICS, GOOGLE_ANALYTICS_TRACKING_ID, CHINESE_LOCATION_INFO_MESSAGE_FIRST, CHINESE_LOCATION_INFO_MESSAGE_SECOND, MAPBOX_TOKEN, MUNICIPALITY_CITIES_ARR, MAP_LAYER_LIST, IS_CHINESE, ROAD_LABEL_DISPLAY, INFO_MESSAGE, RUN_TITLES, USE_ANIMATION_FOR_GRID, USE_DASH_LINE, LINE_OPACITY, MAP_HEIGHT, PRIVACY_MODE, LIGHTS_ON, SHOW_ELEVATION_GAIN, RICH_TITLE, ACTIVITY_TOTAL, TYPES_MAPPING, HOME_PAGE_TITLE, LOADING_TEXT, };
...
export const getRuntimeSingleColor = (typeColor = [MAIN_COLOR, MAIN_COLOR_LIGHT]) => {
    if (typeof window === 'undefined')
        return SINGLE_RUN_COLOR_DARK;
...
export const getCyclingColor = () => {
    return DASHBOARD_RIDE_COLOR;
};
export const getRuntimeRunColor = () => {
...
export const getMapTileVendorStyles = (vendor) => {
    const styles = MAP_TILE_STYLES[vendor];
    return typeof styles === 'object' ? styles : undefined;
};
if (typeof window !== 'undefined') {
    const vendorStyles = getMapTileVendorStyles(MAP_TILE_VENDOR);
    const requiresToken = ['mapbox', 'maptiler', 'stadiamaps'].includes(MAP_TILE_VENDOR);
    if (!vendorStyles) {
        console.error(`Map tile vendor "${MAP_TILE_VENDOR}" is not configured. ` +
            `Available vendors: ${Object.keys(MAP_TILE_STYLES)
                .filter((key) => key !== 'default')
                .join(', ')}`);
    }
    else if (!vendorStyles[MAP_TILE_STYLE_LIGHT]) {
        console.error(`Map tile style "${MAP_TILE_STYLE_LIGHT}" is not valid for "${MAP_TILE_VENDOR}". ` +
            `Available styles: ${Object.keys(vendorStyles).join(', ')}`);
    }
    else if (!vendorStyles[MAP_TILE_STYLE_DARK]) {
        console.error(`Map tile style "${MAP_TILE_STYLE_DARK}" is not valid for "${MAP_TILE_VENDOR}". ` +
            `Available styles: ${Object.keys(vendorStyles).join(', ')}`);
    }
    if (requiresToken && MAP_TILE_ACCESS_TOKEN === '') {
        console.error(`${MAP_TILE_VENDOR} map tiles require MAP_TILE_ACCESS_TOKEN in src/utils/const.ts.`);
    }
}
